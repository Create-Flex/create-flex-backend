...
public interface LegalTaxApi {
    ...
    @Operation(
            summary = "법률/세무 상담 신청",
            description = "크리에이터가 법률 또는 세무 상담을 신청합니다.",
    ...
    @Operation(
            summary = "전체 상담 신청 목록 조회 (관리자용)",
            description = "모든 법률/세무 상담 신청 목록을 조회합니다. 유형 및 상태로 필터링 가능합니다.",
            security = @SecurityRequirement(name = "JWT"),
            responses = {
    ...
    ResponseEntity<List<LegalTaxResponseDTO.Info>> getAllLegalTax(
            @Parameter(description = "상담 유형 필터 (LEGAL/TAX)", example = "TAX")
            @RequestParam(required = false) LegalTaxType type,
            @Parameter(description = "상담 상태 필터 (NOT_RECEIVED/CONTACTED/IN_PROGRESS/DONE)", example = "IN_PROGRESS")
            @RequestParam(required = false) LegalTaxStatus status
    );

    @Operation(
            summary = "내 담당 크리에이터 상담 신청 목록 조회 (매니저용)",
            description = "로그인한 매니저가 담당하는 크리에이터들의 법률/세무 상담 신청 목록을 조회합니다.",
            security = @SecurityRequirement(name = "JWT"),
            responses = {
                    @ApiResponse(responseCode = "200", description = "조회 성공"),
    ...
    ResponseEntity<List<LegalTaxResponseDTO.Info>> getMyLegalTax(
            @Parameter(hidden = true) String userId,
            @Parameter(description = "상담 유형 필터 (LEGAL/TAX)", example = "LEGAL")
            @RequestParam(required = false) LegalTaxType type,
            @Parameter(description = "상담 상태 필터 (NOT_RECEIVED/CONTACTED/IN_PROGRESS/DONE)", example = "CONTACTED")
            @RequestParam(required = false) LegalTaxStatus status
    );

    @Operation(
            summary = "상담 완료 처리 (관리자용)",
            description = "법률/세무 상담을 완료 상태로 변경합니다.",
            security = @SecurityRequirement(name = "JWT"),
            responses = {
    ...
    ResponseEntity<Map<String, Object>> completeLegalTax(
            @Parameter(description = "상담 신청 ID", example = "2001")
